...
1. GitHub issue with quantified validation evidence — anchored so it does not expire (§ *Closing out a fix*).
...
1. **It kills a mutation.** `scripts/discrimination_killmatrix.json` maps node ID → mutations killed:
   ...
   *appends* `_EditableFinder` to `sys.meta_path`, after `PathFinder` — so under `-c` and `-m`, which
   ...
   `_assert_import_is_the_mutated_tree` (`test_discrimination.py:518`) then *confirms* the tree
...
the count. Neither is the list. A test defending something on neither is admissible
...
history rather than over the tree. That is the whole test: **a claim quantified over the current
tree drifts the moment someone fixes it; one quantified over immutable history cannot.** It also
runs backwards — read what an existing claim quantifies over and you know whether it has already
...
Deprecated code MUST immediately redirect to the new standard: (1) old API calls new internally (zero behavior difference); (2) **mandatory equivalence test** (old == new); (3) update ALL call sites (direct, factory, defaults, examples, tests); (4) timeline 3 minor versions OR 6 months before removal. These four clauses are the whole of the policy as this repository holds it. (~~Ref: `DEPRECATION_LIFECYCLE_POLICY.md`~~ — that file was `docs/development/DEPRECATION_LIFECYCLE_POLICY.md` until `cc21745c` moved the internal dev notes out of this repository, so the pointer has named nothing here since. #2359.) Origin: the #616 post-mortem, which IS the file this used to point at — `a6569f57` added `docs/development/DEPRECATION_LIFECYCLE_POLICY.md` as a 596-line document and the post-mortem is its body. The `Ref:` named the right artifact all along; only its path died. It is the worked case for clauses (1) and (2). #490 redirected `conservative=` to `advection_scheme`, kept the old parameter with a deprecation warning, and set the new default to `gradient_upwind`, which leaks mass at the boundary. Callers who had relied on the correct default since #382 silently got the broken one for **34 days**, at a **99.4%** mass error. The post-mortem's own verdict: *"Deprecated with warning but wrong default → 1 month of catastrophic bugs"*, and *"**Equivalence test requirement** would have caught the wrong default immediately"* — quoted with its markdown, because the bold splits the literal and a plain grep for the sentence returns zero. That is why (1) says zero behaviour difference and (2) makes the equivalence test mandatory. `c18c798d` removed the parameter.
