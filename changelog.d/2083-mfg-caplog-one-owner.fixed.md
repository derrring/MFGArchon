- **One owner for mfgarchon log capture in tests** (Issue #2083). `MFGLogger` disables propagation
  ...
  demand and depends on neither the version nor the order. It refuses a logger name no mfgarchon code
  has ever asked for when the block captured nothing, so a misspelt name can no longer satisfy an
  absence assertion. It replaces six hand-rolled collectors and two `logger.warning` spies, and the four
