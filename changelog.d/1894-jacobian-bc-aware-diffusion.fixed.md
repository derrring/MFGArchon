...
Fixed during review: with a volatility **field** and a periodic BC, the periodic wrap entries were formed as `-_diffusion * v` where `_diffusion` is an `(Nx,)` array, so `Jac[i, j] += <array>` raised `ValueError` — and a pre-existing handler meant for the band shapes caught it and replaced the entire Jacobian with `(1/dt) * I`. Measured at `Nx=21`, `sigma(x)=0.3+0.4x`: `nnz` 63 → 21, `np.allclose(J, I/dt)` True, and the backward Newton solve returned a non-root with only a "did not converge" warning. The off-band value now carries the **row's** diffusion (`dRes_i/dU_j = -D_i L[i,j]`), and the wrap assembly moved out of that handler so an assembly bug propagates instead of becoming a plausible wrong answer.

Also fixed during review: the O(Nx) comb tier of `_bc_laplacian_bands` never fired. `pack()` attributed a comb response to every tooth, and a comb is nonzero near *every* tooth, so each tooth collected one spurious off-band entry per other tooth's row — 240 of them at `Nx=21` under no-flux, where the true operator has none. The control vector therefore failed every time and the O(Nx²) fallback always ran: probe count was `Nx + 9` for every `Nx` and every BC. Off-band entries are now read only from columns probed in isolation, which is all a comb can attribute. Probe count is 9 regardless of `Nx`; measured `1311 ms → 3.5 ms` at `Nx=1601`, `332 ms → 1.7 ms` at `Nx=801`. Extraction is unchanged: the bands equal a column-by-column reference exactly (`0.000e+00`) for no-flux, Dirichlet, and both periodic conventions.
