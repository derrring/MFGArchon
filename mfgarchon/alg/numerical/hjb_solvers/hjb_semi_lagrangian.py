...
class HJBSemiLagrangianSolver(BaseHJBSolver):
    ...
    def __init__(
        ...
        # Issue #1547 / RFC #1574 Phase 0 carried a refusal here for a MAXIMIZE control cost, whose
        # alpha* = +grad_u/lambda would have traced the characteristic feet in the wrong direction.
        # #2373 removed the concept, so the condition it tested can no longer be true -- and it had
        # gone SILENTLY dead rather than loudly: keyed on `getattr(cost, "sign", 1) != 1`, the
        # default made it `1 != 1` once the attribute was deleted. The foot velocity is now
        # alpha* = -grad_u/lambda unconditionally.
