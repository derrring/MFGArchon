...
if TYPE_CHECKING:
    ...
    from mfgarchon.config import BaseConfig
...
class BaseMFGSolver(ABC):
    ...
    def _validate_bc_support(self, bc: Any) -> None:
        ...
        if not self.honors_inhomogeneous_neumann:
            ...
            #
            # The "is this datum verifiably zero?" predicate, over BOTH the segments and the
            # fall-through default, is owned by describe_inhomogeneous_bc_data. It used to be a
            # closure here; #1802 wrote a second copy for the same question and reproduced this
            # gate's own #1686 default-channel hole in the process, so there is one now.
            ignored = describe_inhomogeneous_bc_data(bc, bc_types={BCType.NEUMANN})
            if ignored:
