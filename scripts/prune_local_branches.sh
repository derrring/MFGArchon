...
git worktree list --porcelain | awk '/^branch /{sub("refs/heads/","",$2); print $2}' | sort -u > /tmp/_held.txt
...
gh pr list --state merged --limit $((TOTAL + 50)) --json headRefName -q '.[].headRefName' | sort -u > /tmp/_merged.txt
GOT=$(wc -l < /tmp/_merged.txt)
...
gh pr list --state open --limit 200 --json headRefName -q '.[].headRefName' | sort -u > /tmp/_open.txt
...
absorbed() {  # 0 if the branch's unique changes are already present in main
  ...
  git diff "$mb".."$1" > /tmp/_p.diff 2>/dev/null || return 1
  [[ -s /tmp/_p.diff ]] || return 0
  git apply --check --reverse /tmp/_p.diff 2>/dev/null
}

: > /tmp/_kill.txt
printf '%-46s %s\n' "BRANCH" "VERDICT"
while read -r b; do
  if grep -qxF "$b" /tmp/_held.txt;   then printf '%-46s %s\n' "$b" "keep  (held by a worktree)"; continue; fi
  if grep -qxF "$b" /tmp/_open.txt;   then printf '%-46s %s\n' "$b" "keep  (open PR)";            continue; fi
  if grep -qxF "$b" /tmp/_merged.txt; then printf '%-46s %s\n' "$b" "DELETE (merged PR)"; echo "$b" >> /tmp/_kill.txt; continue; fi
  if absorbed "$b";                   then printf '%-46s %s\n' "$b" "DELETE (content in main)"; echo "$b" >> /tmp/_kill.txt; continue; fi
  ...
  case "$st" in
    MERGED) printf '%-46s %s\n' "$b" "DELETE (iteration branch of #$n, merged)"; echo "$b" >> /tmp/_kill.txt ;;
    OPEN)   printf '%-46s %s\n' "$b" "keep  (#$n still open)" ;;
...
COUNT=$(wc -l < /tmp/_kill.txt | tr -d ' ')
echo
if [[ "$DELETE" -eq 0 ]]; then
  echo "dry run: $COUNT branch(es) would be deleted. Re-run with --delete."
  exit 0
fi
{ echo "# deleted $(date +%F); recover with: git branch <name> <sha>"
  while read -r b; do printf '%-46s %s\n' "$b" "$(git rev-parse "$b")"; done < /tmp/_kill.txt
} > "$MANIFEST"
while read -r b; do git branch -D "$b" >/dev/null; done < /tmp/_kill.txt
echo "deleted $COUNT; recovery manifest: $MANIFEST"
