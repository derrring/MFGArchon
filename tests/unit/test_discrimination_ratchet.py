...
def test_every_killer_node_id_still_resolves(td):
    ...
    had been stale since #2181/#2185 touched `test_fp_particle_solver.py`, and the deletion's
    acceptance had to be re-derived by hand instead of read off a gate. TWO of the three are a class
    rename -- file survives, `def` survives, the CLASS moved. The third is not: its class survives and
    the METHOD was deleted with `KDENormalization.INITIAL_ONLY`. An earlier version of this docstring
    ...
    #: detection power: the loop resolves every path from the same root, so a wrong root makes all
    #: 614 read "file gone" and the main assertion fires anyway. It improves the message and nothing
    #: else, and calling it a control overstated it.
...
def test_the_mutation_list_matches_the_parametrisation(td):
    ...
    assert len(td.MUTATIONS) == 24  # 23 on #2145, where mass_drift_reported_as_deviation_from_one is INEFFECTIVE
