...
def test_a_partial_scan_refuses_to_write_and_declines_to_verify(tmp_path, monkeypatch):
    """An environment that cannot import the whole package must not produce a guide.

    The guide is generated by importing and walking `mfgarchon`, and `scan_deprecated` skips an
    unimportable submodule with `logger.debug(); continue` -- so a machine without an optional
    extra sees a smaller package and cannot tell. Measured on an unmodified tree: 72 deduplicated
    deprecations with torch present, 41 without. Two failure modes follow, and this pins both:

    - REGENERATING would delete 31 real deprecations from a user-facing document.
    - CHECKING would fail a correct tree. Wired to the pre-push hook, that blocks every
      contributor on a default install, and the remediation the failure prints is the write above.

    So the scan is strict, and an incomplete one raises `PartialScan`: writing refuses with a
    non-zero exit, and `--check` reports that it did NOT verify and exits 0. A green that means
    "I could not look" has to say so.
    """
    import subprocess
    import sys

    # Reproduce a default install for the child only: a stub whose import raises, exactly as a
    # genuine absence does (`except ImportError` catches `ModuleNotFoundError` too).
    site = tmp_path / "stub_site"
    site.mkdir()
    (site / "torch.py").write_text("raise ImportError('No module named torch (simulated)')\n")

    repo = Path(__file__).resolve().parents[3]
    guide = repo / "docs" / "user" / "DEPRECATION_MODERNIZATION_GUIDE.md"
    before = guide.read_text()
    env = {**os.environ, "PYTHONPATH": str(site)}

    check = subprocess.run(
        [sys.executable, "scripts/generate_deprecation_guide.py", "--check"],
        cwd=repo,
        capture_output=True,
        text=True,
        env=env,
    )
    assert check.returncode == 0, "a check that cannot verify must not fail a correct tree"
    assert "SKIPPED" in check.stderr, "it must say it did not verify"
    assert "not stale" in check.stderr, "and must not let the reader read that as clean"

    write = subprocess.run(
        [sys.executable, "scripts/generate_deprecation_guide.py"],
        cwd=repo,
        capture_output=True,
        text=True,
        env=env,
    )
    assert write.returncode != 0, "refusing to regenerate must be visible to the caller"
    assert guide.read_text() == before, "a partial scan must never rewrite the guide"


def test_strict_scan_refuses_to_skip_an_unimportable_submodule(tmp_path, monkeypatch):
    """`strict=True` turns a skipped submodule into a raise.

    The end-to-end test above pins the refuse/skip OUTCOME, but not this flag: with torch absent
    the failure lands on the top-level import (Issue #1773), which the caller's guard catches
    either way. Mutation-checking that test against `strict=` therefore comes back green, so the
    flag needs its own case -- a package that imports fine and whose SUBMODULE does not, which is
    the situation `strict=` exists for.
    """
    import importlib
    import sys
    ...
    (pkg / "broken.py").write_text("raise ImportError('needs an optional extra')\n")
    (pkg / "fine.py").write_text("")

    monkeypatch.syspath_prepend(str(tmp_path))
    for name in [n for n in sys.modules if n.startswith("synthpkg")]:
        del sys.modules[name]
    mod = importlib.import_module("synthpkg")

    from mfgarchon.utils.deprecation import scan_deprecated

    # Non-strict: the broken submodule is skipped and the caller cannot tell.
    assert scan_deprecated(mod, recursive=True) == []

    with pytest.raises(ImportError, match=r"synthpkg\.broken"):
        scan_deprecated(mod, recursive=True, strict=True)
