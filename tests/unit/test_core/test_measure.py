...
class TestDegenerateCloudWarns1071:
    ...
    Captured with `mfg_caplog` rather than by patching `logger.warning`: the emission is what
    is under test, and a spy on the method cannot tell a real record from a suppressed one.
    """

    LOGGER = "mfgarchon.core.measure"

    def test_1d_zero_spread_warns(self, mfg_caplog):
        with mfg_caplog.at_level(logging.WARNING, logger=self.LOGGER):
        ...
        assert any("zero spread" in m for m in mfg_caplog.messages)

    def test_nd_zero_spread_warns(self, mfg_caplog):
        with mfg_caplog.at_level(logging.WARNING, logger=self.LOGGER):
    ...
    def test_nondegenerate_does_not_warn(self, mfg_caplog):
        with mfg_caplog.at_level(logging.WARNING, logger=self.LOGGER):
            mu = ParticleMeasure(np.array([0.2, 0.5, 0.8]))  # std > 0
            mu.to_density(np.linspace(0.0, 1.0, 51))
        assert not any("zero spread" in m for m in mfg_caplog.messages)
