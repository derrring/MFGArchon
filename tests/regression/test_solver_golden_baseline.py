...
# NOTE: solver_golden_lq_fdm.npz was regenerated 2026-09-14 for #2308. `gradient_upwind` returned the
# one-sided difference of smaller magnitude at a discrete local minimum, where the Godunov momentum for
# |p|^2/2 is 0. This fixture's U has its minimum at column 10 at every sampled time, and that is where
# the diff sits: max|dU| 3.313e-05 (rel 7.81e-04) at (t=0, col 10), max|dM| 6.464e-04 (rel 2.09e-04)
# at (t=3, col 10). The GFDM fixture regenerated byte-identical, the control: that path never reads
# `gradient_upwind`.
#
# NOTE: solver_golden_lq_fdm.npz was regenerated 2026-08-28 for #2145, and `_make_lq_problem` now
# normalises its own initial density. Two changes, and separating them is why the fixture stayed
# interpretable. #1887 removed the constructor's rescale, so the raw Gaussian would have entered at
# mass 0.3236 and every stored value would have shrunk by ~3x -- a 68% diff whose largest element
# sat at the DOMAIN CENTRE, burying the thing this gate should show. Normalising in the fixture (the
# caller's job now) leaves the scheme change alone: max|dU| 6.386e-05 (rel 1.51e-03), max|dM|
# 2.322e-02 (rel 7.51e-03), and the argmax is NODE 0 -- 2.322e-02 at the wall against 1.212e-02
# anywhere inside, decaying inward, which is the signature of a wall control volume and not of an
# interior truncation error.
#
# What the old fixture had frozen in: mass drift 3.466e-03 over the horizon, on the grid's own
# measure. The new one is 1.221e-15. The previous baseline was pinning a 0.35% leak, in the same
# shape as #1900's clamp above -- the gate protecting the defect rather than against it. The GFDM
# fixture regenerated byte-identical, which is the control: the HJB path is untouched by a change to
# the FP wall control volumes.
#
...
def _make_lq_problem(control_cost: float = 1.0) -> MFGProblem:
    ...
    The initial density is normalised HERE, on the grid's own measure. It used to arrive raw and be
    rescaled by the constructor; #1887 removed that rescale -- normalising is the caller's job and
    this fixture is a caller. Doing it explicitly keeps the golden locking a probability density, so
    the fixture's numbers stay interpretable and the diff against the previous baseline is the
    scheme change alone. Left raw, `m_initial` would carry mass 0.3236 and every stored value would
    shrink by ~3x, burying the wall-localised difference this gate exists to see.
    """
    grid = TensorProductGrid(bounds=[(0.0, 1.0)], Nx_points=[21], boundary_conditions=no_flux_bc(dimension=1))
    raw = np.exp(-30 * (np.asarray(grid.coordinates[0]) - 0.5) ** 2)
    scale = float(grid.integrate(raw))
